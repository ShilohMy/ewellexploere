...
const { filterEvidenc } = require('./filters');
...
exports.data = async (req, res) => {
  ...
  if ('tx' in req.body) {
    ...
    console.warn("web3relay block blockNumOrHash: ", blockNumOrHash);
    ...
    */
  } else if ('evidence' in req.body) {
        console.warn("request body: ", req.body);
        var blockNumOrHash;
        console.warn("web3relay evidece blockNumOrHash: ", blockNumOrHash);
        if (/^(0x)?[0-9a-f]{64}$/i.test(req.body.evidence.trim())) {
          blockNumOrHash = req.body.evidence.toLowerCase();
        } else {
          // blockNumOrHash = parseInt(req.body.evidence);
          blockNumber = req.body.evidence;
        }

          Evidence.findOne({ blockNumber: blockNumOrHash }).lean(true).exec('findOne', (err, doc) => {
              if (err || !doc) {
                console.warn("No evidenve data find in this block");
              } else {
